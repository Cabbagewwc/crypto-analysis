...
class NotificationService:
    ...
    def generate_daily_report(
        ...
        # 标题
        report_lines = [
            f"# 📅 {report_date} 加密货币智能分析报告",
            "",
            f"> 共分析 **{len(results)}** 个币种 | 报告生成时间：{datetime.now().strftime('%H:%M:%S')}",
            "",
            "---",
            "",
        ]
    ...
    def generate_dashboard_report(
        ...
        report_lines = [
            f"# 🎯 {report_date} 决策仪表盘",
            "",
            f"> 共分析 **{len(results)}** 个币种 | 🟢买入:{buy_count} 🟡观望:{hold_count} 🔴卖出:{sell_count}",
            "",
            "---",
            "",
        ]
        ...
        for result in sorted_results:
            ...
            # 币种名称（优先使用 dashboard 或 result 中的名称）
            crypto_name = result.name if result.name and not result.name.startswith('Token') else result.code
            
            report_lines.extend([
                f"## {signal_emoji} {crypto_name} ({result.code})",
                "",
            ])
    ...
    def generate_wechat_dashboard(self, results: List[AnalysisResult]) -> str:
        ...
        lines = [
            f"## 🎯 {report_date} 决策仪表盘",
            "",
            f"> {len(results)}个币种 | 🟢买入:{buy_count} 🟡观望:{hold_count} 🔴卖出:{sell_count}",
            "",
        ]
        ...
        for result in sorted_results:
            ...
            # 标题行：信号等级 + 币种名称
            lines.append(f"### {signal_emoji} **{signal_text}** | {crypto_name}({result.code})")
            lines.append("")
    ...
    def generate_wechat_summary(self, results: List[AnalysisResult]) -> str:
        ...
        lines = [
            f"## 📅 {report_date} 加密货币分析报告",
            "",
            f"> 共 **{len(results)}** 个币种 | 🟢买入:{buy_count} 🟡持有:{hold_count} 🔴卖出:{sell_count} | 均分:{avg_score:.0f}",
            "",
        ]
    ...
    def generate_single_crypto_report(self, result: AnalysisResult) -> str:
        """
        生成单个币种的分析报告（用于单币推送模式）
        
        格式精简但信息完整，适合每分析完一个币种立即推送
...
class NotificationBuilder:
    ...
    @staticmethod
    def build_crypto_summary(results: List[AnalysisResult]) -> str:
        """
        构建加密货币摘要（简短版）
...
if __name__ == "__main__":
    ...
    test_results = [
        AnalysisResult(
            ...
            operation_advice='买入',
            technical_analysis='放量突破 MA25，MACD 金叉',
            news_summary='ETF 资金持续流入，市场情绪乐观',
        ),
        AnalysisResult(
            code='ETH/USDT',
            name='Ethereum',
        ...
        AnalysisResult(
            ...
            operation_advice='卖出',
            technical_analysis='跌破 MA25 支撑，量能不足',
            news_summary='链上活跃度下降，DeFi TVL 减少',
        ),
    ]
