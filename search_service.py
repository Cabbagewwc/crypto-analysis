...
职责：
1. 提供统一的新闻搜索接口
2. 支持 Tavily、SerpAPI 和 Bocha 等搜索引擎
3. 多 Key 负载均衡和故障转移
4. 搜索结果缓存和格式化
"""
...
class SearchService:
    ...
    def search_crypto_news(
        ...
        Args:
            ...
            crypto_name: 加密货币名称（如 Bitcoin, Ethereum）
            max_results: 最大返回结果数
        ...
        if focus_keywords is None:
            focus_keywords = [
                "token unlock", "代币解锁",           # 代币经济
                "whale", "巨鲸", "大户",               # 巨鲸动向
                "listing", "上线", "上架",             # 交易所上线
                "airdrop", "空投",                     # 空投消息
                "partnership", "合作", "集成",         # 合作消息
                "hack", "攻击", "漏洞",                # 安全事件
                "SEC", "监管", "regulation",           # 监管消息
                "upgrade", "升级", "更新",             # 技术更新
            ]
        
        # 构建搜索查询（优化搜索效果）
        # 使用英文搜索通常能获取更多加密货币新闻
        query = f"{crypto_name} {symbol} crypto news latest"
        
        logger.info(f"搜索加密货币新闻: {crypto_name}({symbol})")
    ...
    def search_stock_news(
        ...
        """兼容旧接口，内部调用 search_crypto_news"""
        return self.search_crypto_news(stock_code, stock_name, max_results, focus_keywords)
    ...
    def search_crypto_events(
        ...
        if event_types is None:
            event_types = ["token unlock", "airdrop", "listing", "hack", "partnership"]
        
        # 构建针对性查询
        event_query = " OR ".join(event_types)
        query = f"{crypto_name} {symbol} ({event_query})"
        
        logger.info(f"搜索加密货币事件: {crypto_name}({symbol}) - {event_types}")
    ...
    def search_stock_events(
        ...
        """兼容旧接口，内部调用 search_crypto_events"""
        return self.search_crypto_events(stock_code, stock_name, event_types)
    ...
    def search_comprehensive_intel(
        ...
        search_dimensions = [
            {
                'name': 'latest_news',
                'query': f"{crypto_name} {symbol} crypto news latest 2026",
                'desc': '最新消息'
            },
            {
                'name': 'risk_check',
                'query': f"{crypto_name} {symbol} token unlock whale selling hack security risk",
                'desc': '风险排查'
            },
            {
                'name': 'project_updates',
                'query': f"{crypto_name} {symbol} partnership upgrade airdrop ecosystem",
                'desc': '项目进展'
            },
        ]
        
        logger.info(f"开始多维度情报搜索: {crypto_name}({symbol})")
    ...
    def format_intel_report(self, intel_results: Dict[str, SearchResponse], crypto_name: str) -> str:
        """
        格式化情报搜索结果为报告
        ...
        if 'risk_check' in intel_results:
            ...
            if resp.success and resp.results:
                ...
                lines.append("  未发现明显风险信号（Token Unlock、巨鲸抛售、安全事件）")
        
        # 项目进展
        if 'project_updates' in intel_results:
            resp = intel_results['project_updates']
            lines.append(f"\n🚀 项目进展 (来源: {resp.provider}):")
            if resp.success and resp.results:
                for i, r in enumerate(resp.results[:3], 1):
                    lines.append(f"  {i}. {r.title}")
                    lines.append(f"     {r.snippet[:100]}...")
            else:
                lines.append("  未找到项目进展相关信息")
        
        # 兼容旧版的 earnings 字段
        if 'earnings' in intel_results and 'project_updates' not in intel_results:
            resp = intel_results['earnings']
            lines.append(f"\n📊 项目动态 (来源: {resp.provider}):")
            if resp.success and resp.results:
                for i, r in enumerate(resp.results[:3], 1):
                    lines.append(f"  {i}. {r.title}")
                    lines.append(f"     {r.snippet[:100]}...")
    ...
    def batch_search(
        ...
        Args:
            cryptos: 加密货币列表 [{"symbol": "BTC", "name": "Bitcoin"}, ...]
            max_results_per_crypto: 每个加密货币的最大结果数
            delay_between: 每次搜索之间的延迟（秒）
            
        Returns:
            {symbol: SearchResponse} 字典
        """
        results = {}
        ...
        for i, crypto in enumerate(cryptos):
            ...
            symbol = crypto.get('symbol', crypto.get('code', ''))
            name = crypto.get('name', '')
            
            response = self.search_crypto_news(symbol, name, max_results_per_crypto)
...
if __name__ == "__main__":
    ...
    if service.is_available:
        print("=== 测试加密货币新闻搜索 ===")
        response = service.search_crypto_news("BTC", "Bitcoin")
